...
public class MotorControlPanel extends JPanel implements ChangeListener {
	...
	public MotorControlPanel() {
		...
		accelSlider.setToolTipText(Lang.getValue("AccelerationSlider.Tooltip"));
		...
		speedSlider.setToolTipText(Lang.getValue("SpeedSlider.Tooltip"));
		...
		subAccel.add(
				// add a new JLabel above the slider
				new JLabel(Lang.getValue("AccelerationSlider.Label")),
				BorderLayout.NORTH);
		subAccel.add(accelSlider, BorderLayout.CENTER);

		subSpeed.setLayout(new BorderLayout());
		subSpeed.add(
				// add a new JLabel above the sliders
				new JLabel(Lang.getValue("SpeedSlider.Label")),
				BorderLayout.NORTH);
	...
	public void stateChanged(ChangeEvent evt) {
		...
		if (evt.getSource() == speedSlider) {// change the speed.
			...
			synchronized (motors) {
				for (EmulatedMotor mot : motors)
					try {
						...
						System.out
								.println("Uh-Oh! Setting the speed on a motor failed! (in MotorControlPanel)");
						e.printStackTrace();
					}
			}
			...
			synchronized (motors) {
				for (EmulatedMotor mot : motors)
					try {
						...
						System.out
								.println("Uh-Oh! Setting the acceleration on a motor failed! (in MotorControlPanel)");
						e.printStackTrace();
					}
			}
			...
			System.out
					.println("MotorControlPanel: Got a ChangeEvent that wasn't from one of our sliders...");
		}
